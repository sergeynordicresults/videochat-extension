...
function createChatMessagesObserver(subscriberTextOnChanged) {
...
function stateToFoundCountry([_s, c]) {
  return c
}
...
function createGlobalState() {
  ...
  return {
    ...
    //     throw new Error('first should start');
    //   }
    //   observerStopFunction();
    //   observerStopFunction = null;
    // },
    startObserver() {
      if (observerStopFunction) {
      ...
      observerStopFunction = createChatMessagesObserver((textContent) => {
...
(async () => {
  ...
  function ControlPanel() {
    ...
    useEffect(() => {
      console.log('globalState.setOnStateChanged')
    ...
    return createElement(
      ...
      createElement('div', {
        ...
        createElement('span', {
          style: {
            fontWeight: 'bold',
            color: sendAutoplayOnNewFoundUser ? '#27ae60' : '#c0392b',
            userSelect: 'none',
            fontSize: '14px',
          }
  ...
  if (buttonsWrapper) {
    ...
    ReactDOM.createRoot(container).render(createElement(ControlPanel));
  } else {
